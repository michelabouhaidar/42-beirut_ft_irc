...
/*   Updated: 2026/01/02 01:43:58 by mabou-ha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* ---------------- PASS / NICK / USER / CAP / PING ---------------- */

static void cmdPASS(Server &srv, int fd, const IrcMessage &msg)
{
    Client *c = srv.getClient(fd);
    if (!c) return;
...
static void cmdNICK(Server &srv, int fd, const IrcMessage &msg)
{
    Client *c = srv.getClient(fd);
    if (!c) return;
...
static void cmdUSER(Server &srv, int fd, const IrcMessage &msg)
{
    Client *c = srv.getClient(fd);
    if (!c) return;
...
static void cmdPING(Server &srv, int fd, const IrcMessage &msg)
{
    std::string token;
    if (!msg.params.empty())
        token = msg.params[0];
...
static void cmdCAP(Server &srv, int fd, const IrcMessage &msg)
{
    (void)srv; (void)fd; (void)msg;
    // Ignore CAP LS/REQ/END safely for compatibility.
}

/* ---------------- Channels: JOIN / PART / TOPIC / INVITE / KICK / MODE ---------------- */

static void cmdJOIN(Server &srv, int fd, const IrcMessage &msg)
{
...
static void cmdPART(Server &srv, int fd, const IrcMessage &msg)
{
    if (!ensureRegistered(srv, fd)) return;
...
static void cmdTOPIC(Server &srv, int fd, const IrcMessage &msg)
{
    if (!ensureRegistered(srv, fd)) return;
...
static void cmdINVITE(Server &srv, int fd, const IrcMessage &msg)
{
    if (!ensureRegistered(srv, fd)) return;
...
static void cmdKICK(Server &srv, int fd, const IrcMessage &msg)
{
    if (!ensureRegistered(srv, fd)) return;
...
static void cmdMODE(Server &srv, int fd, const IrcMessage &msg)
{
    if (!ensureRegistered(srv, fd)) return;
    ...
    {
        // Minimal: only channel modes here
    ...
    // Query current modes (minimal string)
    if (msg.params.size() == 1)
    {
        std::string modes = "+";
...
/* ---------------- Messaging: PRIVMSG / QUIT ---------------- */

static void cmdPRIVMSG(Server &srv, int fd, const IrcMessage &msg)
{
    if (!ensureRegistered(srv, fd)) return;
...
static void cmdQUIT(Server &srv, int fd, const IrcMessage &msg)
{
    (void)msg;
...
{
    if      (msg.command == "CAP")      cmdCAP(srv, fd, msg);
    else if (msg.command == "PASS")     cmdPASS(srv, fd, msg);
    else if (msg.command == "NICK")     cmdNICK(srv, fd, msg);
    else if (msg.command == "USER")     cmdUSER(srv, fd, msg);
    else if (msg.command == "PING")     cmdPING(srv, fd, msg);

    else if (msg.command == "JOIN")     cmdJOIN(srv, fd, msg);
    else if (msg.command == "PART")     cmdPART(srv, fd, msg);
    else if (msg.command == "TOPIC")    cmdTOPIC(srv, fd, msg);
    else if (msg.command == "INVITE")   cmdINVITE(srv, fd, msg);
    else if (msg.command == "KICK")     cmdKICK(srv, fd, msg);
    else if (msg.command == "MODE")     cmdMODE(srv, fd, msg);

    else if (msg.command == "PRIVMSG")  cmdPRIVMSG(srv, fd, msg);
    else if (msg.command == "QUIT")     cmdQUIT(srv, fd, msg);
    else
    {
        Client *c = srv.getClient(fd);
